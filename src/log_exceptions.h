...
inline void __ThrowCheckImpl(const char* file, const int line, const bool result,
                             const char* expr_str) {
    if (!result) {
        throw TemplateException<std::invalid_argument>(file, line, __GetCheckString(expr_str).c_str());
    }
}
...
inline void __ThrowCheckImplMsg(const char* file, const int line, const bool result,
    ...
    if (!result) {
        ...
        ss << expr_str << " : " << msg;
        std::string m = ss.str();
        throw TemplateException<std::invalid_argument>(file, line, __GetCheckString(m.c_str()));
    }
}

template <typename T1, typename T2>
void __ThrowCheckOpImpl(const char* file, const int line, const bool result, const T1& val1,
                        const T2& val2, const char* val1_str, const char* val2_str,
                        const char* op_str) {
    if (!result) {
        std::stringstream ss;
        ...
        throw TemplateException<std::invalid_argument>(file, line, __GetCheckString(msg.c_str()));
    }
}
...
#define THROW_CUSTOM_CHECK_MSG(condition, exception, msg) \
    if (!condition)                                       \
        throw TemplateException<exception>(               \
            __FILE__, __LINE__, __GetCheckString(#condition) + std::string(" ") + ToString(msg));
